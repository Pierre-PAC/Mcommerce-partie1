...
public class ProductController {
    ...
    public ResponseEntity<Void> ajouterProduit(@Valid @RequestBody Product product) {
        ...
        if (product.getPrix() < 1) {
            throw new ProduitGratuitException(
                    "le prix du produit " + product.getNom() + " ne peut pas etre nul, il doit etre superieure a 1");
        }
    ...
    @ApiOperation(value = "Calcul lamarge sur les produits!")
    @GetMapping(value = "/AdminProduits")
    public ResponseEntity<Object> calculerMargeProduit() {
        List<Product> lproducts = productDao.findAll();
        ...
        for (Product produit : lproducts) {
            results.put(produit.toString(), produit.getPrix() - produit.getPrixAchat());
        }
